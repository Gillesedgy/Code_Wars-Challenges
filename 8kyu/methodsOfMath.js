...
function roundIt(n) {
    ...
    const decimalIndex = str.indexOf('.'); // get the index of the decimal point
    const leftCount = decimalIndex; // count of digits on the left of the decimal point
    const rightCount = str.length - decimalIndex - 1; // count of digits on the right of the decimal point
    
    if (leftCount < rightCount) {
      return Math.ceil(n); // use ceil() method
    } else if (leftCount > rightCount) {
      return Math.floor(n); // use floor() method
    } else {
      return Math.round(n); // use round() method
    }
  }